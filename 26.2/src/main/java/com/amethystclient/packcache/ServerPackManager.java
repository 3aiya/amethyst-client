 ...
 * {@code ensureRunningOnSameThread} call, and Fabric's connection events fire on the render thread. Only the
...
public final class ServerPackManager {
	 ...
	 * {@code handleResourcePackPush}, which passes through our mixin again. This set tells the
	 * mixin to let that one call through untouched.
	 */
	private final Set<UUID> passToVanilla = new HashSet<>();

	// ---- Per-connection state (reset whenever a new Connection is seen) ----
	@Nullable
	 ...
	 */
	public boolean onPackPush(ClientboundResourcePackPushPacket packet, Connection connection, @Nullable ServerData serverInfo,
			Minecraft client, Runnable vanillaHandler) {
		this.initialize();
		...
		if (serverInfo.getResourcePackStatus() == ServerData.ServerPackStatus.DISABLED) {
			// The player explicitly turned server packs off for this server; respect it (vanilla declines or disconnects).
			LOGGER.info("[join] Server resource packs are disabled for '{}' in the server list; using vanilla handling", serverInfo.ip);
			return false;
		}
		...
		String serverKey = PackCache.serverKey(serverInfo.ip);
		...
		if (current != null && current.serverKey().equals(serverKey) && current.sha1().equals(serverSha1)) {
			...
			sendStatus(connection, packId, ServerboundResourcePackPacket.Action.ACCEPTED);
			sendStatus(connection, packId, ServerboundResourcePackPacket.Action.DOWNLOADED);
			sendStatus(connection, packId, ServerboundResourcePackPacket.Action.SUCCESSFULLY_LOADED);
			return true;
		}
		...
		if (onDisk != null && onDisk.sha1().equals(serverSha1)) {
			...
			this.activate(onDisk, client).thenAccept(ok -> sendStatus(connection, packId,
					ok ? ServerboundResourcePackPacket.Action.SUCCESSFULLY_LOADED : ServerboundResourcePackPacket.Action.FAILED_RELOAD));
			return true;
		}
		...
		this.downloader.download(url, serverSha1, target, PackDownloader.vanillaHeaders(client))
				.whenCompleteAsync((file, error) -> {
					...
					if (!sameSession || !connection.isConnected()) {
	...
	public void onPackRemove(ClientboundResourcePackPopPacket packet, Connection connection) {
		this.beginSessionIfNew(connection);
	 ...
	 */
	public void onJoinedServer(Connection connection, @Nullable ServerData serverInfo, Minecraft client) {
		this.initialize();
		this.beginSessionIfNew(connection);
	 ...
	 */
	private CompletableFuture<Boolean> activate(@Nullable PackCache.CachedPack pack, Minecraft client) {
		this.activePack = pack;
		if (pack != null) {
			this.cache.writeLastServer(pack.serverKey());
		}
		return client.reloadResourcePacks().handle((ignored, error) -> {
	...
	private void fallBackToVanilla(String serverKey, UUID packId, Connection connection, Runnable vanillaHandler) {
	...
	private void beginSessionIfNew(Connection connection) {
	...
	private static void sendStatus(Connection connection, UUID packId, ServerboundResourcePackPacket.Action status) {
		if (connection.isConnected()) {
			connection.send(new ServerboundResourcePackPacket(packId, status));
		}
	}
